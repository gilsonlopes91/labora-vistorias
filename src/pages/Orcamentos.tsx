/* Controle de orçamentos: carteira comercial da organização.
   Indicadores no topo, filtros, e a lista com troca rápida de status, registro
   de recebimento e geração do PDF da proposta. */
...
import { getModeloProposta, type ModeloProposta } from '@/services/modelosProposta'
...
export default function Orcamentos() {
  ...
  const trocarStatus = async (orcamento: Orcamento, status: StatusOrcamento) => {
    try {
      ...
      // Carimba as datas do ciclo quando o status muda para elas.
  ...
  const gerarPdf = async (orcamento: Orcamento) => {
    ...
    try {
      ...
      let modelo: ModeloProposta | null = orcamento.expand?.modelo_proposta_id || null
      if (!modelo && orcamento.modelo_proposta_id) {
        modelo = await getModeloProposta(orcamento.modelo_proposta_id)
      }
      if (!modelo) {
        toast.error('Escolha um modelo de proposta no orçamento antes de gerar o PDF')
        return
      }
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Propostas comerciais das empresas cadastradas, do rascunho ao recebimento.
          </p>
        </div>
